...
import Top3Card from "./_components/Top3Card";
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const personIdParsed = parseInt(personId);
        const fetchedPerson = await getPersonById(personIdParsed);

        if (!fetchedPerson) {
          redirect("/");
        }

        setCurrentPerson(fetchedPerson);

        const persons = await getAllPersons();
        const top3 = persons.sort((a, b) => b.points - a.points).slice(0, 3);
        const restPersons = persons
          .filter((person) => !top3.includes(person))
            ...
            <>
              ...
              <div className="bg-white rounded-xl mb-12">
                <div className="table-container">
                      ...
                      return (
                        ...
                        >
                          <div style={{ minWidth: "1.8rem" }}>
                            <span>
                              {index + 4 + currentPage * itemsPerPage}
                            </span>
                          </div>
                  ...
                  {itemsPerPage % 2 === 0 && (
                    <hr className="table-bottom-line" />
                  )}
                </div>
